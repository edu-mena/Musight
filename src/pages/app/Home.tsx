    ...
    <div className="px-4 py-5 space-y-6">
      ...
      <div>
        ...
        {loading ? (
          ...
          <Link to={`/app/debates/${hotDebate.id}`} className="card-app block p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-2 mb-2">
              <span className="pill bg-primary/10 text-primary">{hotDebate.category}</span>
              <span className="text-[10px] text-muted-foreground font-mono-accent">{hotDebate.date}</span>
            </div>
            <h3 className="font-display font-bold text-base leading-snug">{hotDebate.title}</h3>
            <p className="text-sm text-muted-foreground mt-1 leading-relaxed line-clamp-2">{hotDebate.summary}</p>
            <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><Users size={11} className="text-orange-500" />{hotDebate.participants}</span>
              <span>{hotDebate.experts} especialistas</span>
              <span>{hotDebate.comment_count ?? 0} comentários</span>
            </div>
          </Link>
        ) : (
          <div className="card-app p-6 text-center text-sm text-muted-foreground">Em breve novos debates.</div>
        )}
      </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="card-app p-6 text-center text-sm text-muted-foreground">Em breve novos artigos.</div>
        ) : (
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    {a.has_audio && (
                      <span className="flex items-center gap-1 text-[10px] text-orange-500 font-semibold">
                        <Headphones size={10} />Audio {a.audio_duration}
                      </span>
                    )}
      ...
      <div className="card-app p-4 grid grid-cols-3 gap-2 text-center">
        ...
        <div className="border-x border-border">
          <div className="font-display font-bold text-xl text-primary">{user?.contributions ?? 0}</div>
