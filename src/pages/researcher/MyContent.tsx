...
function ActionRow({
  ...
  const editPath =
    kind === "a" ? `/researcher/publicar-artigo?edit=${id}` : `/researcher/criar-debate?edit=${id}`;
  ...
  return (
    <div className="flex items-center gap-1 pt-2 border-t border-border mt-1">
      <Link
        to={editPath}
        className="text-xs font-semibold text-muted-foreground hover:text-foreground px-2 py-1 flex items-center gap-1"
      >
      ...
      <button
        onClick={onConfirm}
        className="text-xs font-semibold text-red-600 hover:text-red-700 px-2 py-1 flex items-center gap-1 ml-auto"
      >
...
export const MyContent = () => {
  ...
  const deleteDebate = async (id: number) => {
    try {
      ...
      toast.error("Erro ao eliminar debate.");
    }
    setConfirmDelete(null);
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
        {typeFilters.map((f) => (
          <FilterPill key={f.id} active={typeF === f.id} onClick={() => setTypeF(f.id)}>
            {f.label}
          </FilterPill>
        ))}
      </div>
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
        {statusFilters.map((f) => (
          <FilterPill key={f.id} active={statusF === f.id} onClick={() => setStatusF(f.id)}>
            {f.label}
          </FilterPill>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card-app p-10 flex flex-col items-center text-center gap-3">
          ...
          <Link to="/researcher/publicar-artigo" className="btn-primary">
            Publicar o primeiro artigo
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          {visibleDebates.map((d) => {
            const st = statusMeta[d.status as keyof typeof statusMeta] ?? {
              label: d.status,
              cls: "bg-muted text-muted-foreground",
            };
            return (
