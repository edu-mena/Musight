...
export const ManageDebates = () => {
  ...
  useEffect(() => {
    api.get<ApiDebate[]>("/admin/debates")
      .then(setItems)
      .catch(() => {})
  ...
  const approve = async (id: number) => {
    ...
    } catch { setItems((xs) => xs.map((x) => x.id === id ? { ...x, status: "em_revisao" } : x)); toast.error("Erro ao aprovar."); }
  };

  const reject = async (id: number) => {
    if (!rejectReason.trim()) return;
    const reason = rejectReason;
    setItems((xs) => xs.map((x) => x.id === id ? { ...x, status: "recusado", rejection_reason: reason } : x));
    try {
      await api.put(`/admin/debates/${id}/reject`, { reason });
      toast("Debate rejeitado.", { description: "O autor será notificado." });
    } catch { setItems((xs) => xs.map((x) => x.id === id ? { ...x, status: "em_revisao", rejection_reason: null } : x)); toast.error("Erro ao rejeitar."); }
    setRejectReason("");
    setActiveReject(null);
  };

  const cancelReject = () => { setActiveReject(null); setRejectReason(""); };

  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-4">
      <header>
        <h1 className="font-display font-bold text-2xl md:text-3xl">Debates</h1>
        <p className="text-sm text-muted-foreground">Revê e aprova propostas de debate dos pesquisadores.</p>
      </header>
      ...
      {loading ? (
        ...
        <div className="card-app p-10 text-center text-sm text-muted-foreground">Sem debates nesta categoria.</div>
      ) : (
              ...
              <article key={d.id} className="card-app p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`pill ${statusPill[d.status] ?? "bg-muted text-muted-foreground"}`}>
                  ...
                  isRejecting ? (
                    <div className="p-3 rounded-xl border border-red-200 bg-red-50 space-y-2">
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={cancelReject} className="text-xs font-semibold text-muted-foreground px-3 py-1.5">Cancelar</button>
                        <button onClick={() => reject(d.id)} disabled={!rejectReason.trim()}
                    ...
                    <div className="flex gap-2 pt-2 border-t border-border mt-1">
                      ...
                      <button onClick={() => { setActiveReject(d.id); setRejectReason(""); }}
                ...
                {d.status === "recusado" && d.rejection_reason && (
                  <div className="p-2 rounded-lg bg-red-50 border border-red-100">
                    <p className="text-[11px] text-red-700"><span className="font-semibold">Motivo: </span>{d.rejection_reason}</p>
                  </div>
                )}
              </article>
            );
